...
class PathAliasDefinition {
    ...
    provideDefinition(document, position, token) {
        ...
        const index = (0, common_1.getIndexOfWorkspaceFolder)(document.uri);
        if (index === undefined)
            return null;
        if (range) {
            const inputPath = document.getText(range);
            const resPath = inputPath.slice(1, -1);
            const mostLike = (0, common_1.mostLikeAlias)(this._aliasList[index], resPath.split('/')[0]);
            if (mostLike) {
    ...
    importDefinition(document, position) {
        ...
        const wsIndex = (0, common_1.getIndexOfWorkspaceFolder)(document.uri);
        ...
        if (execResult) {
            ...
            const mostLike = (0, common_1.mostLikeAlias)(this._aliasList[wsIndex], pathAlias.split('/')[0]);
            if (mostLike) {
                const pathList = [
                ...
                if (extname === 'js' || extname === 'ts') {
                    ...
                    // 这里是已经导入的函数或变量
                    const exportIdentifierList = (0, traverseSourceFile_1.traverse)(absolutePathWithExtname, file);
