...
class PathAlias {
    constructor(ctx) {
        ...
        if (vscode_1.workspace.workspaceFolders && vscode_1.workspace.getWorkspaceFolder.length) {
            vscode_1.workspace.workspaceFolders.forEach((ws) => {
                (0, watcher_1.generateWatcher)(ws.uri.fsPath);
            });
        }
        ...
        const handler = (0, common_1.debounce)(() => {
            this.updateStatInfo();
        }, 1000);
        exports.eventBus
            .on("file-change", (path) => {
    ...
    recollectDependencies(document) {
        ...
        while ((execResult = importReg.exec(content))) {
            ...
            const mostLike = (0, common_1.mostLikeAlias)(this._aliasList[index], pathAlias.split("/")[0]);
            if (mostLike) {
                ...
                if (!extname) {
                    ...
                    else if (fs.existsSync((0, common_1.normalizePath)(absolutePath))) {
                        absolutePath += "/index";
                        extname = "js";
                    }
                }
        ...
        this._functionTokenList = (0, getSignatureFromFile_1.getFunctionSignatureFromFiles)(this._importAbsolutePathList);
    ...
    initStatInfo() {
        if (vscode_1.workspace.workspaceFolders && vscode_1.workspace.workspaceFolders.length) {
            ...
            vscode_1.workspace.workspaceFolders.forEach((ws, index) => {
                ...
                this._aliasMap[index] = Object.assign(Object.assign({}, this._aliasMap[index]), (0, config_1.getAliasConfig)(ws.uri.fsPath || ""));
                Object.keys(this._aliasMap[index]).forEach((alias) => {
                    const realPath = this._aliasMap[index][alias].replace("${cwd}", ws.uri.fsPath || "");
                    let isLegal = true;
                    if (isLegal && !(0, fs_1.existsSync)(realPath)) {
                    ...
                    else if (isLegal && !(0, fs_1.statSync)(realPath).isDirectory()) {
...
function aliasStatInfo(alias, realPath) {
    ...
    stat["children"] = (0, fs_1.readdirSync)(realPath).reduce((pre, currentPath) => {
...
function getStatInfo(name, absolutePath) {
    ...
    if ((0, fs_1.statSync)(absolutePath).isDirectory()) {
        resStatInfo["children"] = (0, fs_1.readdirSync)(absolutePath).reduce((pre, curName) => {
